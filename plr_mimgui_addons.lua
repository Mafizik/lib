...
module.ToggleButton = function(str_id, bool)
	...
	-- ИСПРАВЛЕНО: Убрали (+ radius) из ширины InvisibleButton, чтобы текст встал ближе
	...
	draw_list:AddRectFilled(imgui.ImVec2(p.x, p.y + (height / 6)), imgui.ImVec2(p.x + width, p.y + (height - (height / 6))), col_bg, 10.0)
	draw_list:AddCircleFilled(imgui.ImVec2(p.x + (radius * 0.65) + t * (width - (radius * 1.3)), p.y + radius), radius - 1.0, imgui.ColorConvertFloat4ToU32(module._SETTINGS.ToggleButton.colors[bool[0] and 1 or 2]))
